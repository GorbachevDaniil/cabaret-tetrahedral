...
void Mesh::fillNodeNodeRelation() {
    for (unsigned long i = 0; i < cells.size(); i++) {
        ...
        for (auto i_n = innerNodeIDToVector.cbegin(); i_n != innerNodeIDToVector.cend(); ++i_n) {
            for (auto j_n = innerNodeIDToVector.cbegin(); j_n != innerNodeIDToVector.cend(); ++j_n) {
                if (i_n == j_n) {
                    continue;
                }

                Eigen::Vector3d vectorProduct;
                vectorProduct[0] = i_n->second[1] * j_n->second[2] - i_n->second[2] * j_n->second[1];
                vectorProduct[1] = i_n->second[2] * j_n->second[0] - i_n->second[0] * j_n->second[2];
                vectorProduct[2] = i_n->second[0] * j_n->second[1] - i_n->second[1] * j_n->second[0];
                if (std::abs(vectorProduct.norm()) <= 10e-15) {
                    cell->nodeIDToOppositeNodeID[i_n->first] = j_n->first;
                }
            }
        }
